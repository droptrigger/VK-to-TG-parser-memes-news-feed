...
Бот, который с помощью vk-api ~~ворует~~ копирует мемы вам в чат, ЛС или супергрппу!  
...
В свою очередь новые посты проверяются спецаильным скриптом [cheker](bot/checker.py).  
...
* `list` - Выводит список всех групп, на которые подписан чатю
...
### 1. Для начала вам потребутеся склонировать репозиторий 
